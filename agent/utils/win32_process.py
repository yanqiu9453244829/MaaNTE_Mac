...
# This module uses Windows-only APIs.
# On non-Windows platforms, only no-op stubs are exposed so that callers
# that do a conditional import do not crash.
if not sys.platform.startswith("win"):
    def _noop(*args, **kwargs):
        return None

    find_game_process = _noop
    get_pids_by_name = _noop
    find_windows_by_process = _noop
    find_window_by_process = _noop
    get_client_size = _noop
    get_window_rect = _noop
    resize_window = _noop
    resize_client_area = _noop
    ensure_process_client_size = _noop
    ensure_game_window_resolution = _noop
    get_window_text = _noop
    get_class_name = _noop
else:
    ...
    # DPI awareness: prevents GetClientRect returning scaled virtual coords at 150% scaling
    ...
    def find_window_by_process(
        process_name, hwnd_class=None, require_title=False,
        selected_hwnd=0, last_hwnd=0,
    ):
        windows = find_windows_by_process(
            process_name, hwnd_class=hwnd_class, require_title=require_title,
        )
        if not windows:
            return None
        selected = next(
    ...
    def resize_window(hwnd, width, height, center=True):
        """Resize the outer window and center it."""
        if not hwnd:
            return False
        width = int(width)
        height = int(height)
        ...
        if center:
            ...
            if not user32.SetWindowPos(
                hwnd, None, expected_left, expected_top, 0, 0,
    ...
    def ensure_process_client_size(
        process_name, width, height, center=True, tolerance=2,
        settle_ms=300, hwnd_class=None, require_title=False,
        selected_hwnd=0, last_hwnd=0,
    ):
        """Find a process window and resize its client area to the target size."""
        hwnd = find_window_by_process(
            process_name, hwnd_class=hwnd_class, require_title=require_title,
            selected_hwnd=selected_hwnd, last_hwnd=last_hwnd,
        )
        if not hwnd:
            return {"success": False, "reason": "window_not_found", "hwnd": None, "before": None, "after": None}
        target = (int(width), int(height))
        before = get_client_size(hwnd)
        if before is None:
            return {"success": False, "reason": "client_size_unavailable", "hwnd": hwnd, "before": None, "after": None}
        if abs(before[0] - target[0]) <= tolerance and abs(before[1] - target[1]) <= tolerance:
            return {"success": True, "reason": "already_matched", "hwnd": hwnd, "before": before, "after": before}
        resized = resize_client_area(hwnd, target[0], target[1], center=center, tolerance=tolerance)
        if resized and settle_ms:
            kernel32.Sleep(int(settle_ms))
        after = get_client_size(hwnd)
        if resized:
            _log(f"game window resolution {before[0]}x{before[1]} -> {target[0]}x{target[1]}")
        return {
            "success": bool(resized),
            "reason": "resized" if resized else "resize_failed",
            "hwnd": hwnd, "before": before, "after": after,
        }
    ...
    def ensure_game_window_resolution(
        ...
        **kwargs,
        ...
        return ensure_process_client_size(process_name, width, height, settle_ms=settle_ms, **kwargs)
